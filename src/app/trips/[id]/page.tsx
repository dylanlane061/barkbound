...
export default async function TripPage({ params }: { params: { id: string } }) {
  // These three queries only depend on the trip id, so run them concurrently
  // instead of awaiting one after another.
  const [tripRows, nodes, itinerary] = await Promise.all([
    db.select().from(trips).where(eq(trips.id, params.id)),
    db
      .select()
      .from(tripNodes)
  ...
  // Fetch every place inside ANY node's bounding box in a single query, then group
  // them back to nodes in memory — avoids the previous one-query-per-node (N+1).
  const nodeBoxes = nodes.map((node) => ({
    node,
    bbox: radiusToBbox(node.latitude, node.longitude, node.radiusMiles),
  }));
  ...
  const nodeResults = nodeBoxes.map(({ node, bbox }) => ({
    node,
    places: allPlaces
      .filter(
        (p) =>
